...
`home/` 同理，新增文件需在 `home/default.nix` 的 `imports` 中添加引用。
...
两个 Wayland 窗口管理器，通过 **Ly** (TUI 登录管理器) 在登录时选择：

| WM | 默认 Shell | 系统配置 | 用户配置 |
|---|---|---|---|
| **Niri** (滚动平铺) | Noctalia Shell | `modules/desktop/niri.nix` | `home/niri.nix` |
| **Hyprland** (动态平铺) | Caelestia Shell | `modules/desktop/hyprland.nix` | `home/hyprland.nix` |
...
**必须使用 `home.activation` 脚本**（而非 `xdg.configFile`）将默认配置以真实文件格式复制到目标位置，且仅在文件不存在或为符号链接时复制。参考 `home/migration.nix`。

### Niri + Fcitx5 启动时序

Fcitx5 依赖 Niri 的 `zwp_input_method_v2` 协议。过早启动会导致静默注册失败，终端无法输入中文。当前方案三要素：
1. `home/default.nix` 中用 `Hidden=true` 覆盖 XDG 自动启动
2. `home/niri.nix` 中用 `spawn-at-startup "sh" "-c" "sleep 3 && fcitx5 -d"` 延迟启动
3. Niri `environment {}` 块中设置后备环境变量

### Dracula 主题保护

Caelestia Shell 切换壁纸时会通过 dconf 覆盖 GTK 主题为 `adw-gtk3-dark`。两层保护：
1. Hyprland 环境变量 `GTK_THEME=Dracula`（优先级高于 dconf）
2. `home/theming.nix` 中 `dconf.settings` 锁定 Dracula 值
