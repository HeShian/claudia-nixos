...

{
  ...
  nix.settings = {
    ...
    # --- Nix 商店优化（由下方定时任务处理，而非每次操作后执行）---
    # auto-optimise-store 改为定时运行以减少 IO
  };
  ...
  # 定时执行 store 优化替代 auto-optimise-store（减少每次构建后的 IO 开销）
  nix.optimise = {
    automatic = true;
    dates = [ "04:00" ];                  # 每天凌晨 4 点运行
  };
